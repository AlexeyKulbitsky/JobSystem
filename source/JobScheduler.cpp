...
namespace jobs
{
    ...
    JobSheduler& JobSheduler::getInstance()
    {
        static JobSheduler instance;
        return instance;
    }
    ...
    void JobSheduler::WaitForJob(Job* job)
    {
        while(job->unfinishedJobs > 0)
        {
            Job* childJob = m_workers[0]->GetJob();
            if (childJob)
            {
                childJob->function(childJob, childJob->args);
                Finish(childJob);
                /*childJob->unfinishedJobs.fetch_sub(1);
                if (childJob->parent)
                {
                    childJob->parent->unfinishedJobs.fetch_sub(1);
                }*/
            }
        }
    }
